...
const DEVICE_SIGNAL_URL = null; // Set to your external device endpoint to enable hardware signaling.
let localStream;
...
socket.on('session-invite', ({ from, sessionId }) => {
    ...
    // Trigger external device vibration/signal if configured
    if (DEVICE_SIGNAL_URL) {
        axios.get(`${DEVICE_SIGNAL_URL}/incomingCall`)
            .then(response => console.log('Signal sent', response.data))
            .catch(e => console.error('Signal error', e));
    } else {
        console.debug('External device signaling disabled.');
    }
...
socket.on('invite-rejected', ({ userId }) => {
    alert('User refused the invitation. Leaving the session.');
    leaveSession();
});

socket.on('disconnect', () => {
    alert('Disconnected from the signaling server. The call has been ended.');
    leaveSession();
});
